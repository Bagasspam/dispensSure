...
const MQTT_CLIENT_ID = `nodejs_dashboard_backend_${Math.random().toString(16).substr(2, 8)}`; // ID unik untuk klien backend
...
function connectMqtt() {
    ...
    mqttClient = mqtt.connect(`mqtts://${MQTT_BROKER_HOST}:${MQTT_BROKER_PORT}`, mqttOptions);
    ...
    mqttClient.on('connect', () => {
        ...
        mqttClient.subscribe(MQTT_TOPIC_DATA, (err) => {
            if (!err) {
                ...
                console.error(`[MQTT] Gagal berlangganan topik ${MQTT_TOPIC_DATA}:`, err);
            }
        });
    });
